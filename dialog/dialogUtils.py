...
'''put embedding of each word in a sentence '''
...
'''
Generate a reply given features extracted from the input sentences.
'''
...
def AttributeCatcher(FEs, FTs, sentenceIntent, entity, assignmentM, polarity, testSent):
    ...
    def stopWordCleaner(listedSentence):
        cleaned = []
        badWords = ['this', 'that', 'the', 'it', 'of', 'shall', 'be', 'as']
        for token in listedSentence:
            if not (token in badWords):
                cleaned.append(token)
        return cleaned
    
    def tokenUniter(listedTokens): 
        return ' '.join(word for word in listedTokens).lower().replace(' ', '_') 
        
    def labelSentGen():
        catchedAttribute= 'init'
        if not polarity:
            catchedAttribute = {'label': tokenUniter(stopWordCleaner(assignmentM))}
        if polarity:
            catchedAttribute = {'label':None}
        return catchedAttribute

    def ownershipSentGen():
        catchedAttribute= 'init'
        if not polarity:
            catchedAttribute = {'ownership': tokenUniter(stopWordCleaner(assignmentM))}
        if polarity:
            catchedAttribute = {'ownership':None}
        return catchedAttribute

    def positionSentGen():
        catchedAttribute= 'init'
        if not polarity:
            catchedAttribute = {'location': tokenUniter(stopWordCleaner(assignmentM))}
        if polarity:
            catchedAttribute = {'location':None}
        return catchedAttribute

    def restrictionsSentGen():
        catchedAttribute= 'init'
        
        sentence = nltk.word_tokenize(testSent)
        
        # for this class instead of looking only at polarity, we use semantic of sentence.
        if assignmentM[0] == 'good':
            assignmentSemantic = True
        else:
            assignmentSemantic = False 
        ...
        # eg. do not touch this chair, touch this chair have Towards label.
        # firstly taking out words that have Towards labels
        towardElements = []
        for i in range(len(FEs)):
            if 'Towards' in FEs[i][0]:
                towardElements.append(sentence[i])
        
        # Secondly take out the VB from Towards.
        restr = ' '.join(word for word in towardElements)
        rest = nlp(restr)
        verbs=[]
        for token in rest:
            if 'VB' in token.tag_:
                verbs.append(token.text)
        #Thirdly, make a string from verbs
        verbsUnited = '_'.join(word for word in verbs)
        ...
        if not semantic:
            catchedAttribute = {'restriction': verbsUnited}
        if semantic:
            catchedAttribute = {'restriction': 'doNot'+verbsUnited}    
    ...
    def functionalitySentGen():
        ...
        if (not semantic):
            catchedAttribute ={'functionality':'notWorking'}
        if semantic:
            catchedAttribute = {'functionality': 'working'} 
        return catchedAttribute

    def weightSentGen():
        catchedAttribute= 'init'
        if not polarity:
            catchedAttribute = {'weight': tokenUniter(stopWordCleaner(assignmentM))}
        if polarity:
            catchedAttribute = {'weight': 'not_'+tokenUniter(stopWordCleaner(assignmentM))}
        return catchedAttribute

    def sizeSentGen():
        sent= 'init'
        if not polarity:
            catchedAttribute = {'size': tokenUniter(stopWordCleaner(assignmentM))}
        if polarity:
            catchedAttribute = {'size': 'not_'+tokenUniter(stopWordCleaner(assignmentM))}
        return catchedAttribute

    
    if sentenceIntent == 'Labeling':  
        catchedAttribute = labelSentGen()
        
    elif sentenceIntent == 'Possession':
        catchedAttribute = ownershipSentGen()
        
    elif sentenceIntent == 'BeingLocated':
        catchedAttribute = positionSentGen()
                    
    elif sentenceIntent == 'HavingOrLackingAccess':
        catchedAttribute = restrictionsSentGen()
    
    elif sentenceIntent == 'Weight':
        catchedAttribute = weightSentGen()
            
    elif sentenceIntent == 'Size':            
        catchedAttribute = sizeSentGen()
        
    elif sentenceIntent == 'Being_operational':
        #print ('THISI S ISSSSSSSSSSSSSS')
        catchedAttribute = functionalitySentGen()
    else:
        catchedAttribute = None #'I could not understand the sentence'   
